...
if (!class_exists("WP_Translate")) {
	class WP_Translate {
		...
		const WPTOPTIONS = "wpTranslateOptions";
		
		public function __construct() {
			$this->plugin_name = plugin_basename(__FILE__);
			$this->current_version = '5.3.0';
		...
		public function wpTranslate_options_check() {
			...
			if(!get_option(self::WPTOPTIONS)) {
				...
				add_option(self::WPTOPTIONS, $wpTranslateOptions);
			}
			else {
				$wpTranlsateOptions	= get_option(self::WPTOPTIONS);
				...
				update_option(self::WPTOPTIONS, $wpTranlsateOptions);
			}
		}
		...
		public function translate_Init() {
			$wpTranslateOptions = get_option(self::WPTOPTIONS);
			$doTranslate = true;
		...
		public function wp_translate_settings() {
			...
			update_option(self::WPTOPTIONS, $wpTranslateOptions);
